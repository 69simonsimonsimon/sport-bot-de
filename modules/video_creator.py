...
Kein tmp-File, keine PTS-Probleme, kein Duration-Bug.
"""
...
def _sanitize(text: str) -> str:
    """Umlaute → ASCII, Emoji entfernen, ffmpeg-Sonderzeichen raus."""
    text = (text
...
def create_video(clip_path: Path, audio_path: Path, title: str,
    ...
    accent = {"fussball": "0x00AAFF", "nba": "0xFF6B00", "nfl": "0x00CC55"}.get(sport, "0x00AAFF")
    label  = {"fussball": "FUSSBALL", "nba": "NBA", "nfl": "NFL"}.get(sport, "SPORT")
    ...
    if words:
        for w in words:
            ...
            # Dynamische Schriftgröße — lange Wörter (z.B. Deutsch) schrumpfen
            wlen = len(wtext)
    ...
    vf_parts = [
        # ── Letterbox-Skalierung (kein Strecken) ─────────────────────────────
        f"scale={W}:{H}:force_original_aspect_ratio=decrease",
        f"pad={W}:{H}:(ow-iw)/2:(oh-ih)/2:black",
        # ── Leichte Vignette ──────────────────────────────────────────────────
        f"drawbox=x=0:y=0:w={W}:h={H}:color=black@0.12:t=fill",
        # ── Unterer Gradient für Untertitel-Lesbarkeit ────────────────────────
        ...
        # ── Sport-Badge (oben links) ──────────────────────────────────────────
        f"drawbox=x=28:y={badge_y}:w=250:h=68:color=black@0.60:t=fill",
        ...
        # ── Handle ────────────────────────────────────────────────────────────
        f"drawtext=text='SynCinSportDE'{fa}:fontsize=24:fontcolor=white@0.55"
        f":x=(w-text_w)/2:y={handle_y}",
    ]

    # Karaoke-Wort-Filter anhängen
    ...
    # List-Datei neben der Output-Datei ablegen
    ...
    try:
        r = _run([
            "ffmpeg", "-y",
            # +genpts regeneriert Timestamps — behebt kaputte PTS vom -c copy Concat
            "-fflags", "+genpts",
            # Input 0: Video (via concat-Loop)
            "-f", "concat", "-safe", "0", "-i", str(list_file),
            # Input 1: Audio
            "-i", str(audio_path),
            # Filter: Video-Chain auf Stream 0
    ...
    # Duration prüfen — nicht Dateigröße (statische Clips sind legitim klein)
    actual_dur = _get_duration(output_path)
    if actual_dur < 20.0:
        logger.error(f"[video] Output zu kurz ({actual_dur:.1f}s, {mb:.1f} MB) — ffmpeg stderr: {r.stderr[-400:]}")
        raise RuntimeError(f"Video zu kurz ({actual_dur:.1f}s) — Encoding fehlgeschlagen")
