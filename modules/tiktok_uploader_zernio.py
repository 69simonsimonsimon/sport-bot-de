...
def _upload_to_bunny(video_path: str) -> str:
    """Video zu Bunny.net Storage hochladen — gibt öffentliche CDN-URL zurück."""
    ...
    if not password or not cdn_url:
        raise ValueError("BUNNY_STORAGE_PASSWORD / BUNNY_CDN_URL nicht gesetzt")
...
def _upload_to_host(video_path: str) -> str:
    ...
    # 0. Bunny.net (primär — zuverlässig, keine IP-Blöcke)
    try:
...
def _create_post(video_url: str, caption: str, thumbnail_url: str = "",
    ...
    # YouTube-Titel: immer #Shorts enthalten damit YouTube es als Short erkennt
...
def upload_video_zernio(video_path: str, caption: str, thumbnail_path: str = "",
    ...
    """
    # Nur wirklich leere Dateien ablehnen (statische Videos sind legitim klein)
    size_mb = Path(video_path).stat().st_size / 1_048_576
    if size_mb < 0.1:
        logger.error(f"   ✗ Video leer ({size_mb:.2f} MB) — korrupt, Upload abgebrochen")
        return False
